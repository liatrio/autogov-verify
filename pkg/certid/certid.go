...
// categorizes the cert-identity list into latest, approved, and revoked
...
type Options struct {
	...
	URL string
	// type of identities to validate against
	Type IdentityType
	// disables caching
	DisableCache bool
...
func (v *Validator) IsValidIdentity(certIdentity string) (bool, error) {
	...
	// but only for branch references / does not alter commit shas
	...
	case TypeLatest:
		...
		return false, fmt.Errorf("certificate identity not found in latest list")
	case TypeApproved:
		...
		return false, fmt.Errorf("certificate identity not found in approved list")
	case TypeAll:
		// check latest
		for _, id := range v.list.Latest {
			if id.Identity == normalizedIdentity || id.Identity == certIdentity {
		...
		return false, fmt.Errorf("certificate identity not found in latest or approved lists")
	default:
		return false, fmt.Errorf("unsupported identity type: %s", v.options.Type)
	}
}
...
// returns all valid identities according to the configured type
func (v *Validator) GetValidIdentities() ([]Identity, error) {
	...
	case TypeAll:
		...
		return validIdentities, nil
	default:
		return nil, fmt.Errorf("unsupported identity type: %s", v.options.Type)
	}
}
